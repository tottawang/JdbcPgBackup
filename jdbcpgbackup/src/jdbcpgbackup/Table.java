...
final class Table extends DbBackupObject {
  ...
  static class TableFactory implements DBOFactory<Table> {
    ...
    public Iterable<Table> getDbBackupObjects(Connection con, Schema schema) throws SQLException {
      ...
      try {
        stmt = con.prepareStatement("SELECT pg_get_userbyid(c.relowner) AS tableowner, "
            + "c.relname AS tablename, c.oid AS table_oid " + "FROM pg_class c "
    ...
    @Override
    public Table getDbBackupObject(Connection con, String tableName, Schema schema)
        throws SQLException {
      return getDbBackupObject(con, tableName, schema, false);
    }

    public Table getDbBackupObject(Connection con, String tableName, Schema schema,
        boolean loadColumns) throws SQLException {
      Table table = null;
      PreparedStatement stmt = null;
      try {
        stmt = con.prepareStatement(
            "SELECT pg_get_userbyid(c.relowner) AS tableowner, c.oid AS table_oid "
                + "FROM pg_class c " + "WHERE c.relkind = 'r'::\"char\" AND c.relnamespace = ? "
                + "AND c.relname = ?");
        stmt.setInt(1, schema.getOid());
    ...
    private void loadColumns(Connection con, Table table, int tableOid) throws SQLException {
      PreparedStatement stmt = null;
      try {
        stmt = con.prepareStatement("SELECT a.attname,a.atttypid,"
  ...
  static class CachingTableFactory extends CachingDBOFactory<Table> {
    ...
    protected PreparedStatement getAllStatement(Connection con) throws SQLException {
      return con.prepareStatement("SELECT c.relnamespace AS schema_oid, c.relname AS tablename, "
          + "pg_get_userbyid(c.relowner) AS tableowner, c.oid " + "FROM pg_class c "
          + "WHERE c.relkind = 'r'::\"char\"");
    }

    @Override
    protected Table newDbBackupObject(Connection con, ResultSet rs, Schema schema)
        throws SQLException {
      Table table = new Table(rs.getString("tablename"), schema, rs.getString("tableowner"));
    ...
    private void loadColumns(Connection con) throws SQLException {
      ...
      PreparedStatement stmt = null;
      try {
        stmt = con.prepareStatement("SELECT a.attrelid AS table_oid, a.attname, a.atttypid,"
            + "a.attnotnull OR (t.typtype = 'd' AND t.typnotnull) AS attnotnull, a.atttypmod, "
            + "row_number() OVER (PARTITION BY a.attrelid ORDER BY a.attnum) AS attnum, "
            + "pg_catalog.pg_get_expr(def.adbin, def.adrelid) AS adsrc, t.typtype "
            + "FROM pg_catalog.pg_attribute a "
        ...
        while (rs.next()) {
          ...
          if (table != null) {
            table.columns.add(table.new Column((BaseConnection) con, rs));
            if (++count % 100000 == 1)
              ZipBackup.debug("loaded " + count + " columns");
          }
        }
        rs.close();
      } finally {
  ...
  /*
   * private static void loadSequences(Connection con, Schema schema, Map<String,Table> tables)
   * throws SQLException { ZipBackup.timerStart("load sequences"); PreparedStatement stmt =
   * con.prepareStatement(
   * "SELECT c.relname AS sequencename, d.refobjsubid AS columnid, p.relname AS tablename " +
   * "FROM pg_class c, pg_depend d, pg_class p " + "WHERE d.refobjid = p.oid " +
   * "AND p.relnamespace = (SELECT oid FROM pg_namespace WHERE nspname = ?) " +
   * "AND c.oid = d.objid AND c.relkind = 'S'"); stmt.setString(1, schema.getName()); ResultSet rs =
   * stmt.executeQuery(); while (rs.next()) {
   * tables.get(rs.getString("tablename")).columns.get(rs.getInt("columnid")).setSequenceName(rs.
   * getString("sequencename")); } rs.close(); stmt.close(); ZipBackup.timerEnd("load sequences"); }
   */
  ...
  private static final Set<String> appendSizeTo = new HashSet<String>(Arrays.asList("bit", "varbit",
      "bit varying", "bpchar", "char", "varchar", "character", "character varying"));

  private static final Set<String> appendPrecisionTo =
      new HashSet<String>(Arrays.asList("time", "timestamp", "timetz", "timestamptz"));

  /*
   * private static final Map<String,String> aliasMap = new HashMap<String,String>(); static {
   * aliasMap.put("serial","integer"); aliasMap.put("serial4","integer");
   * aliasMap.put("serial8","bigint"); aliasMap.put("bigserial","bigint"); }
   * 
   * private static String getAlias(String type) { String alias = aliasMap.get(type); return alias
   * != null ? alias : type; }
   */
  ...
  private class Column {
    ...
    private Column(BaseConnection con, ResultSet rs) throws SQLException {
      ...
      nullable = rs.getBoolean("attnotnull") ? java.sql.DatabaseMetaData.columnNoNulls
          : java.sql.DatabaseMetaData.columnNullable;
      String columnDef = rs.getString("adsrc");
      if (columnDef != null) {
        defaultValue = columnDef.replace("nextval('" + schema.getName() + ".", "nextval('"); // remove
                                                                                             // schema
                                                                                             // name
      ...
      if (isAutoincrement) {
        PreparedStatement stmt = null;
        try {
          stmt = con.prepareStatement("SELECT pg_get_serial_sequence( ? , ? ) AS sequencename");
