...
final class View extends DbBackupObject {
  ...
  static class ViewFactory implements DBOFactory<View> {
    ...
    public Iterable<View> getDbBackupObjects(Connection con, Schema schema) throws SQLException {
      ...
      try {
        stmt = con.prepareStatement("SELECT * FROM pg_views WHERE schemaname = ?");
        stmt.setString(1, schema.getName());
        ResultSet rs = stmt.executeQuery();
        while (rs.next()) {
          views.add(new View(rs.getString("viewname"), schema, rs.getString("viewowner"),
    ...
    @Override
    public View getDbBackupObject(Connection con, String viewName, Schema schema)
        throws SQLException {
      View view = null;
      PreparedStatement stmt = null;
      try {
        stmt = con.prepareStatement("SELECT * FROM pg_views WHERE schemaname = ? AND viewname = ?");
        stmt.setString(1, schema.getName());
    ...
    protected final PreparedStatement getAllStatement(Connection con) throws SQLException {
      return con.prepareStatement(
          "SELECT c.relnamespace AS schema_oid, c.relname AS viewname, pg_get_userbyid(c.relowner) AS viewowner, "
              + "pg_get_viewdef(c.oid) AS definition " + "FROM pg_class c "
              + "WHERE c.relkind = 'v'::\"char\"");
      /*
       * "SELECT * FROM pg_views " + "WHERE schemaname NOT LIKE 'pg_%' " +
       * "AND schemaname <> 'information_schema'");
       */
    }

    @Override
    protected final View newDbBackupObject(Connection con, ResultSet rs, Schema schema)
        throws SQLException {
      return new View(rs.getString("viewname"), schema, rs.getString("viewowner"),
          rs.getString("definition"));
    }
