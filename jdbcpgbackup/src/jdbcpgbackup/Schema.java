...
final class Schema extends DbBackupObject {
  ...
  static class SchemaFactory implements DBOFactory<Schema> {
    ...
    @Override
    public Schema getDbBackupObject(Connection con, String schemaName, Schema ignored)
        throws SQLException {
      PreparedStatement stmt = null;
      Schema schema = null;
      try {
  ...
  static class CachingSchemaFactory implements DBOFactory<Schema> { // does not extend
                                                                    // CachingDBOFactory
    ...
    @Override
    public Collection<Schema> getDbBackupObjects(Connection con, Schema ignored)
        throws SQLException {
      if (map == null) {
        loadMap(con);
      }
    ...
    @Override
    public Schema getDbBackupObject(Connection con, String name, Schema ignored)
        throws SQLException {
      if (map == null) {
        loadMap(con);
      }
    ...
    private void loadMap(Connection con) throws SQLException {
      ...
      try {
        ...
        while (rs.next()) {
          ...
          map.put(schemaName,
              new Schema(rs.getString("schemaname"), rs.getString("owner"), rs.getInt("oid")));
        }
        rs.close();
      } finally {
        if (stmt != null)
  ...
  static Schema createSchema(Connection con, String schemaName, String owner,
      DBOFactory<Schema> schemaFactory) throws SQLException {
    PreparedStatement stmt = null;
    try {
      stmt = con.prepareStatement("CREATE SCHEMA " + schemaName + " AUTHORIZATION " + owner);
      stmt.executeUpdate();
    } finally {
      if (stmt != null)
        stmt.close();
