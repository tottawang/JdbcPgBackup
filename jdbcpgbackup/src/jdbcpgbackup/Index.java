...
class Index extends DbBackupObject {
  ...
  static class IndexFactory implements DBOFactory<Index> {
    ...
    public Iterable<Index> getDbBackupObjects(Connection con, Schema schema) throws SQLException {
      ...
      try {
        ...
        while (rs.next()) {
          indexes.add(new Index(rs.getString("indexname"), schema, rs.getString("tablename"),
       ...
       * ZipBackup.timerStart("primary keys"); DatabaseMetaData metaData = con.getMetaData(); rs =
       * metaData.getPrimaryKeys(null, schema.getName(), null); Map<String,PrimaryKey> pkeys = new
       * HashMap<String,PrimaryKey>(); while (rs.next()) { String tableName =
       * rs.getString("TABLE_NAME"); String indexName = rs.getString("PK_NAME"); PrimaryKey pkey =
       * pkeys.get(indexName); if (pkey == null) { pkey = new PrimaryKey(indexName, schema,
       * tableName); pkeys.put(indexName, pkey); }
       * pkey.columns.put(Integer.valueOf(rs.getInt("KEY_SEQ")), rs.getString("COLUMN_NAME")); }
       * rs.close(); indexes.addAll(pkeys.values()); ZipBackup.timerEnd("primary keys");
       */
      return indexes;
    }

    @Override
    public Index getDbBackupObject(Connection con, String indexName, Schema schema)
        throws SQLException {
      Index index = null;
      PreparedStatement stmt = null;
      try {
        ...
        ResultSet rs = stmt.executeQuery();
        if (rs.next()) {
          index = new Index(rs.getString("indexname"), schema, rs.getString("tablename"),
              rs.getString("indexdef"));
        } else {
  ...
  static class CachingIndexFactory extends CachingDBOFactory<Index> {
    ...
    protected CachingIndexFactory(Schema.CachingSchemaFactory schemaFactory,
    ...
    @Override
    protected Index newDbBackupObject(Connection con, ResultSet rs, Schema schema)
        throws SQLException {
      Table table = tableFactory.getTable(rs.getInt("table_oid"));
      return new Index(rs.getString("indexname"), schema, table.getName(),
          rs.getString("indexdef"));
    }
  ...
  /*
   * not used because too slow getting them private final static class PrimaryKey extends Index {
   * 
   * private SortedMap<Integer,String> columns = new TreeMap<Integer,String>();
   * 
   * private PrimaryKey(String name, Schema schema, String tableName) { super(name, schema,
   * tableName, null); }
   * 
   * @Override public String getSql() { StringBuilder buf = new StringBuilder();
   * buf.append("ALTER TABLE "); buf.append(tableName); buf.append(" ADD CONSTRAINT ");
   * buf.append(name); buf.append(" PRIMARY KEY ("); for (String column : columns.values()) {
   * buf.append(column); buf.append(","); } buf.deleteCharAt(buf.length()-1); buf.append(");\n");
   * return buf.toString(); }
   * 
   * }
   */
